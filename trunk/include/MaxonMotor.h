...
public:
	...
	bool setMotorParameters(void * keyHandle, unsigned long &errorCode);
	bool setVelocity(void * keyHandle, long int velocity, unsigned
			long &errorCode);
	bool setRelativePosition(void * keyHandle, long int relPosition,
			unsigned long &errorCode);
	bool isMoving(void * keyHandle, unsigned long &errorCode);
	void readInMotorParameters(string paramFile);
	string getMotorName();
	void printParameterData();

protected:
	//A string that says if the motor has been initialized yet
	bool isInitialized;
